...
-----------------------
Handles all LLM interactions. Supports Gemini (google-genai SDK) and Groq.
"""
...
class LLMHandler:
    def __init__(self, api_key: str, model_name: str = "gemini-2.0-flash", provider: str = "gemini"):
        ...
        if provider == "gemini" and GEMINI_AVAILABLE and api_key:
            ...
            logger.warning(f"LLM provider '{provider}' not available or missing API key.")
...
Return JSON only."""
    ...
    def generate_rag_answer(self, query: str, context_chunks: list) -> Tuple[str, Optional[str]]:
        ...
        prompt = f"""Answer based ONLY on this context. If answer is not in context, say "I don't have that information in the hostel policy documents."
CONTEXT:
{context}
QUESTION: {query}
ANSWER:"""
